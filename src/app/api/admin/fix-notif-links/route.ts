...
export async function POST() {
  try {
    ...
    // 3. Fix admin SYSTEM notifications candidature → /admin?tab=producers
    const adminNotifs = await prisma.notification.updateMany({
      where: {
        type: 'SYSTEM',
        title: { contains: 'candidature' },
        link: { not: '/admin?tab=producers' },
      },
      data: { link: '/admin?tab=producers' },
    })
    results.adminNotifs = adminNotifs.count
